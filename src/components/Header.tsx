      ...
      <div className="mx-auto flex max-w-[1280px] flex-col gap-2 px-4 py-3 sm:h-16 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-8 sm:py-0">
        ...
        <div className="flex items-center justify-center gap-1 sm:gap-2">
          <button
            onClick={onPrevDay}
            aria-label="이전 날짜"
            className="grid h-9 w-9 place-items-center rounded-lg text-body hover:bg-subtle focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
          >
        ...
        {/* 우: 현재 시각 */}
        <div className="hidden text-right text-[14px] text-muted tabular-nums sm:block sm:min-w-[72px]">
          {now ? formatClock(now) : ""}
        </div>
      </div>
    </header>
